...
helper_check_heap(char *msg) {
  ...
  TracePrintf(4,"| %s HEAP check_heap(\"%s\") %x - %x\n",WHOSE_HEAP,msg,heap_start,heap_end);

  if (not_initialized) {
    TracePrintf(4,"| %s HEAP heap not initialized\n",WHOSE_HEAP);
    if (USER_LIB)
      init_heap();
    else {
  ...
  TracePrintf(4,"| %s HEAP total size = 0x%x\n", WHOSE_HEAP, heap_end - heap_start);
  ...
  TracePrintf(4,"| %s HEAP Blocks: 0x%x (0x%x dirty)\n", WHOSE_HEAP,blocks,dirty);
  TracePrintf(4,"| %s HEAP Bytes: 0x%x free, 0x%x used, 0x%x ifrag\n", WHOSE_HEAP,
	      free_block_bytes, used_block_bytes, ifrag_bytes);


  TracePrintf(4,"| %s HEAP check done\n",WHOSE_HEAP);
...
split_free_block(mblock_t *bp, u_long len) {
  ...
  if (bp->size_used) {
    TracePrintf(4, "heap problem: splitting nonempty block\n");
    return;
  }

  if (bp->size < len) {
    TracePrintf(4, "heap problem: splitting block that's too small\n");
    return;
  }
